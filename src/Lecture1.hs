...
lowerAndGreater n =
    ...
    where
        ...
        getResult lower greater (x:xs)
            | x > n = getResult (lower + 1) greater xs
            | x < n = getResult lower (greater + 1) xs
            | otherwise = getResult lower greater xs
